<?php
session_start();
//$_SESSION['idp'] = (isset($_SESSION['idp']) && !empty($_SESSION['idp']))?$_SESSION['idp']:0;
...
elseif($sect=='b_apply_now'){
	...
	$emailReceive = 0; $estimating = 0;	
	if($userType != 'inspector'){
		$addProj = isset($_POST['addProject']) ? 1 : 0; 
		$emailReceive = isset($_POST['emailReceive']) ? 1 : 0; 
		$estimating = isset($_POST['estimating']) ? 1 : 0; 
	}
	$messageBoard = isset($_POST['messageBoard']) ? 1 : 0;
	$documentRegister = isset($_POST['documentRegister']) ? 1 : 0;
	$healthCheck = isset($_POST['healthCheck']) ? 1 : 0;
	$projects = isset($_POST['projects']) ? 1 : 0;
	$reports = isset($_POST['reports']) ? 1 : 0;
	$inspections = isset($_POST['inspections']) ? 1 : 0;	
		...
		if($obj->db_num_rows($obj->db_query($q)) > 0){
			...
			if($obj->db_num_rows($obj->db_query($q)) > 0){
				...
				$q = "INSERT INTO user SET
								...
								user_type = '".$userType."',
								message_board_perm = '".$messageBoard."',
								document_register_perm = '".$documentRegister."',
								health_check_perm = '".$healthCheck."',
								projects_perm = '".$projects."',
								reports_perm = '".$reports."',
								inspections_perm = '".$inspections."',
								estimating_perm = '".$estimating."',
...
if($userType == 'manager'){
	...
	/*$tabPermissionQuery = "INSERT INTO user_permission (User_id, permission_name, is_allow, created_by, created_date, last_modified_by, last_modified_date)VALUES
	('".$newUserId."', 'message_board', '".$messageBoard."', '".$_SESSION['ww_c_id']."', NOW(), '".$_SESSION['ww_c_id']."', NOW()),
	('".$newUserId."', 'document_register', '".$documentRegister."', '".$_SESSION['ww_c_id']."', NOW(), '".$_SESSION['ww_c_id']."', NOW()),
	('".$newUserId."', 'health_check', '".$healthCheck."', '".$_SESSION['ww_c_id']."', NOW(), '".$_SESSION['ww_c_id']."', NOW()),
	('".$newUserId."', 'projects', '".$projects."', '".$_SESSION['ww_c_id']."', NOW(), '".$_SESSION['ww_c_id']."', NOW()),
	('".$newUserId."', 'reports', '".$reports."', '".$_SESSION['ww_c_id']."', NOW(), '".$_SESSION['ww_c_id']."', NOW()),
	('".$newUserId."', 'inspections', '".$inspections."', '".$_SESSION['ww_c_id']."', NOW(), '".$_SESSION['ww_c_id']."', NOW()),
	('".$newUserId."', 'estimating', '".$estimating."', '".$_SESSION['ww_c_id']."', NOW(), '".$_SESSION['ww_c_id']."', NOW())";
	mysql_query($tabPermissionQuery);*/
...
elseif($sect=='c_dashboard_edit'){
	...
	$tradingName = mysql_real_escape_string(trim($_POST['tradingName']));
	$acn = mysql_real_escape_string(trim($_POST['acn']));
	$abn = mysql_real_escape_string(trim($_POST['abn']));


	if($fname=='' || $compname=='' || $email=='' || $mobile=='' || $bus_l1=='' || $bus_suburb=='' || 		$bus_state=='' || $bus_post=='' || $bus_country=='' || $bil_l1=='' || $bil_suburb=='' || $bil_state=='' ||  $bil_post=='' || $bil_country=='' || $username==''){
		...
		if($obj->db_num_rows($obj->db_query("SELECT * FROM ".COMPANIES." WHERE comp_userName='$username' AND c_id!='".$_SESSION['ww_c_id']."'")) > 0){
			...
			$q = "UPDATE pms_companies SET
							comp_userName = '$username',
							comp_fullname = '$fname',
							comp_name = '$compname',
							comp_email = '$email',
							comp_mobile = '$mobile',
							comp_businessadd1 = '$bus_l1',
							comp_billadd1 = '$bil_l1',
							comp_businessadd2 = '$bus_l2',
							comp_billadd2 = '$bil_l2',
							comp_bussuburb = '$bus_suburb',
							comp_billsuburb = '$bil_suburb',
							comp_businessstate = '$bus_state',
							comp_billstate = '$bil_state',
							comp_businesspost = '$bus_post',
							comp_billpost = '$bil_post',
							comp_businesscountry = '$bus_country',
							comp_billcountry = '$bil_country',
...
//			$result = 2; //			sleep(1);
			...
			//New Permision Setup
			$permArr = array('General Consultant', 'Services Engineer', 'Structural Engineer', 'Architect', 'Client', 'Tenancy Fitout', 'Penthouse Architecture', 'Landscaping');	
			
			$qs = "SELECT project_id, user_role, issued_to, is_pdf, sub_user_role, architect_attr_2, drawing_module  FROM user_projects WHERE is_deleted = 0  AND user_id = '".$_SESSION['ww_builder_id']."'";
			$rs = $obj->db_query($qs);
			$drawModuleArr = array();
			$projUserRole = array();
			while($f = $obj->db_fetch_assoc($rs)){	$drawModuleArr[$f["project_id"]] = $f["drawing_module"];
				$_SESSION['userRole'] = $f["user_role"];
				$_SESSION['subUserRole'] = $f["sub_user_role"];
				$_SESSION['architectAttrTwo'] = $f["architect_attr_2"];
				$projUserRole[$f["project_id"]] = $f["user_role"];
				if(in_array($f["user_role"], $permArr)){
					$_SESSION['idp'] = $f["project_id"];
				}elseif($_SESSION['userRole'] == 'Tender'){
					$_SESSION['idp'] = $f["project_id"];
				}else{
					$_SESSION['idp'] = $f["project_id"];
				}
				//$_SESSION['idp'] = isset($_SESSION['idp'])?$_SESSION['idp']:0;
				#$_SESSION['idp'] = $f["project_id"];
				if ($f["user_role"] == "Sub Contractor"){
					$_SESSION['userIssueTo'] = $f["issued_to"];
					break;
				}else if($f["user_role"] == "General Consultant"){
					$_SESSION['is_pdf'] = $f["is_pdf"];
					break;
				}
			...
			$_SESSION['drawingModuleType'] = $drawModuleArr;
...
//Check Cookies are set or not and set it to session variables
			if(isset($_SESSION['inspViewPath']) && !empty($_SESSION['inspViewPath'])){
				header("location:".$_SESSION['inspViewPath']);
			}else{
				#Start:- New Tab permissions
				if($_SESSION['ww_builder']['health_check_perm'] == 1){
					header("location:pms.php?sect=b_full_analysis");
					
				}else if($_SESSION['ww_builder']['message_board_perm'] == 1){
					header("location:pms.php?sect=messages&id=".base64_encode($_SESSION['idp']));
					
				}else if($_SESSION['ww_builder']['document_register_perm'] == 1){
					header("location:pms.php?sect=drawing_register&type=pmb&id=".base64_encode($_SESSION['idp']));
					
				}else if($_SESSION['ww_builder']['inspections_perm'] == 1){
					header("location:pms.php?sect=i_defect");	
					
				}else if($_SESSION['ww_builder']['reports_perm'] == 1){
					header("location:pms.php?sect=i_report");	
					
				}else if($_SESSION['ww_builder']['projects_perm'] == 1){
					header("location:pms.php?sect=show_project");	
					
				}else if($_SESSION['ww_builder']['estimating_perm'] == 1){
					header("location:pms.php?sect=estimate");	
					
				}else{
					header("location:pms.php?sect=unkownTab");	
				}
				#End:- New Tab permissions	
				
				/*
				if($_SESSION['userRole'] != 'Sub Contractor'){
					if(in_array($_SESSION['userRole'], $permArr)){
						header("location:pms.php?sect=messages&id=".base64_encode($_SESSION['idp']));
					}elseif($_SESSION['userRole'] == 'Tender'){
						header("location:pms.php?sect=drawing_register&type=pmb&id=".base64_encode($_SESSION['idp']));
					}else{
...
//			if($userInfo['created_by']==$userInfo['last_modified_by'] && $userInfo['last_modified_date']==$userInfo['created_date'] && $_SESSION['ww_builder']['user_type']=='inspector'){
			if(isset($_SESSION['inspViewPath']) && !empty($_SESSION['inspViewPath'])){
				header("location:".$_SESSION['inspViewPath']);
			}else{
				
				#Start:- New Tab permissions
				if($userInfo['last_modified_by']==0 && $_SESSION['ww_builder']['user_type']=='inspector'){	
					header("location:pms.php?sect=b_changepassword");
					
				}elseif($_SESSION['ww_builder']['health_check_perm'] == 1){
					header("location:pms.php?sect=b_full_analysis");
				...
				if($userInfo['last_modified_by']==0 && $_SESSION['ww_builder']['user_type']=='inspector'){	
					header("location:pms.php?sect=b_changepassword");
				}elseif($_SESSION['userRole'] != 'Sub Contractor'){
					if(in_array($_SESSION['userRole'], $permArr)){
						header("location:pms.php?sect=messages&id=".base64_encode($_SESSION['idp']));
					}elseif($_SESSION['userRole'] == 'Tender'){
						header("location:pms.php?sect=drawing_register&type=pmb&id=".base64_encode($_SESSION['idp']));
