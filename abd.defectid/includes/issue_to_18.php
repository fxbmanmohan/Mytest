				...
				for($i=1;$i<$num;$i++) //read second line beacuse first line cover headings
				{
					if(!empty($fieldarray[$i][1]))
					{
						...
						else
						{
							...
							@$insert="insert into inspection_issue_to (issue_to_name,company_name,issue_to_phone,issue_to_email,project_id,last_modified_date,last_modified_by,created_date,created_by) values ('".addslashes(trim($fieldarray[$i][1]))."','".addslashes(trim($fieldarray[$i][0]))."','".trim($fieldarray[$i][2])."','".trim($fieldarray[$i][3])."',".$_SESSION['idp'].",now(),".$builder_id.",now(),".$builder_id.")";
...
<div style="width:722px; height:50px; float:left; margin-top:5px;">
	<form method="post" name="csvIssueto" id="csvIssueto" enctype="multipart/form-data">
		<table width="690px" border="0" cellspacing="0" cellpadding="3">
			<tr>
				<td colspan="4" align="left"><a href="/csv/Issued_To_Template.csv" style="text-decoration:none;color:#FFF;"><strong style="font-size:16px;">Click here to download CSV template</strong></a></td>
			</tr>   
			<tr>
