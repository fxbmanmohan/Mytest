...
if($obj->db_num_rows($qd)>0){	
	...
	if(isset($_POST['update_x'])){
		...
		$certificate_of_occupancy = $_POST['certificate_of_occupancy'] !='' ? date('Y-m-d', strtotime(str_replace("/", "-", $_POST['certificate_of_occupancy']))) : '';
		$open_date = $_POST['open_date'] !='' ? date('Y-m-d', strtotime(str_replace("/", "-", $_POST['open_date']))) : '';
		$proposed_completion = $_POST['proposed_completion'] !='' ? date('Y-m-d', strtotime(str_replace("/", "-", $_POST['proposed_completion']))) : '';
		$actual_completion = $_POST['actual_completion'] !='' ? date('Y-m-d', strtotime(str_replace("/", "-", $_POST['actual_completion']))) : '';
...
window.onload = function(){
	new JsDatePick({
		useMode:2,
		target:"certificate_of_occupancy",
		dateFormat:"%d/%m/%Y"
	});
	new JsDatePick({
		useMode:2,
	...
	new JsDatePick({
		useMode:2,
		target:"proposed_completion",
		dateFormat:"%d/%m/%Y"
	});
	new JsDatePick({
		useMode:2,
		target:"actual_completion",
		dateFormat:"%d/%m/%Y"
	});	
};
</script>
					...
					<div class="signin_form">
						<table width="470" border="0" align="left" cellpadding="0" cellspacing="15">
							...
							<tr>
								...
								<td><input name="certificate_of_occupancy" id="certificate_of_occupancy" class="input_small" readonly="readonly" value="<?php echo ($f['certificate_of_occupancy']!='0000-00-00')?date('d/m/Y', strtotime($f['certificate_of_occupancy'])):'';?>"/></td>
							</tr>  
							<tr>
								<td valign="top">Open date</td>
								<td><input name="open_date" id="open_date" class="input_small" readonly="readonly" value="<?php echo ($f['open_date']!='0000-00-00')?date('d/m/Y', strtotime($f['open_date'])):'';?>"/></td>
							</tr>  
							<tr>
								<td valign="top">Proposed completion</td>
								<td><input name="proposed_completion" id="proposed_completion" class="input_small" readonly="readonly" value="<?php echo ($f['proposed_completion']!='0000-00-00')?date('d/m/Y', strtotime($f['proposed_completion'])):'';?>"/></td>
							</tr>  
							<tr>
								<td valign="top">Actual completion</td>
								<td><input name="actual_completion" id="actual_completion" class="input_small" readonly="readonly" value="<?php echo ($f['actual_completion']!='0000-00-00')?date('d/m/Y', strtotime($f['actual_completion'])):'';?>"/></td>
							</tr>                                
							<tr>
