...
if(isset($_POST['removeProject'])){
	...
	$deleteQry = "UPDATE project_inspections SET is_deleted = '1' WHERE inspection_id = '".$df_id."' AND project_id = '".$projectId."'";
	mysql_query($deleteQry);
	for($i=0;$i<sizeof($_POST["issue_to_id"]);$i++){
		$issue_to_id = $_POST['issue_to_id'][$i];
		$deleteMulQry = "UPDATE issued_to_for_inspections SET is_deleted = '1' WHERE issued_to_inspections_id = '".$issue_to_id."'";
		mysql_query($deleteMulQry);
	}?>
	<script language="javascript" type="text/javascript">window.location.href="?sect=i_defect&ms=<?=base64_encode('Deleted')?>";</script>
<?php
}
...
$issueToSelect = $object->selQRYMultiple('issue_to_name', 'inspection_issue_to', 'project_id = '.$inspectionDetail['project_id'].' and is_deleted=0 group by issue_to_name');
	$issToList = '';?>
<div class="content_center" style="margin-left:70px;">
...
$checkListStatus = $object->checklistStatus($inspectionDetail['project_id'], $df_id); ?>
		<div class="content_hd" style="background-image:url(images/edit_defect_hd.png);margin: -5px 0 -30px -80px"></div>
