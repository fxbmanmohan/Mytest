...
*/	
	...
	$issueToData = $obj->selQRYMultiple('*', "master_issue_to_contact", " is_deleted = '0' AND contact_id IN(".implode(',',$_POST['assignIssueTo']).") ".$oldIds." GROUP BY company_name");	
	...
	<form method="post" name="issueToForm" id="issueToForm" action="">
		<table border="0" cellspacing="0" cellpadding="3" style="width:748px;">
			<tr>
                ...
                <?php 
					$issueToProjData = $obj->selQRYMultiple('master_contact_id, issue_to_name, company_name', 'inspection_issue_to', " project_id = '".$_SESSION['idp']."' and is_deleted = '0' and issue_to_name!='' and company_name!='' group by company_name");
					$projIssueToArr = array();
					foreach($issueToProjData as $issueTo){
						$projIssueToArr[$issueTo['master_contact_id']] = $issueTo['issue_to_name'];
					}
					
					$issueToData = $obj->selQRYMultiple('contact_id, issue_to_name, company_name', 'master_issue_to_contact', " is_deleted = '0' and issue_to_name!='' and company_name!='' group by company_name");
					foreach($issueToData as $issueTo){
