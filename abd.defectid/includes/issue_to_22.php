	...
	if(isset($_FILES['csvFile']['name']) && !empty($_FILES['csvFile']['name']))
	{
		...
		if($ext=='csv' || $ext=='CSV')
		{
			...
			for($i=1;$i<$num;$i++) //read second line beacuse first line cover headings
			{
				...
				{
					$select="select * from inspection_issue_to where issue_to_name='".$fieldarray[$i][1]."' AND project_id=".$_SESSION['idp']." and is_deleted=0";
					//echo $select; die; 
					...
					{
						@$insert="insert into inspection_issue_to (issue_to_name,company_name,issue_to_phone,issue_to_email,project_id,last_modified_date,last_modified_by,created_date,created_by) values ('".trim($fieldarray[$i][1])."','".trim($fieldarray[$i][0])."','".trim($fieldarray[$i][2])."','".trim($fieldarray[$i][3])."',".$_SESSION['idp'].",now(),".$builder_id.",now(),".$builder_id.")";
						mysql_query($insert);
...
<table width="100%" border="0" align="left" cellpadding="5" cellspacing="0">
		<tr>
               ...
               <!-- <br /><a href="pms.php?sect=project_inspections" <?php if($_GET['sect'] == 'project_inspections'){echo 'class="left_btn5active"';}?> class="left_btn5" ><br />
				</a>-->
                </td>
...
<div style="width:722px; height:50px; float:left; margin-top:5px;">
	<form method="post" name="csvIssueto" id="csvIssueto" enctype="multipart/form-data">
		<table width="690px" border="0" cellspacing="0" cellpadding="3">
			<tr>
				<td colspan="3" align="left"><a href="/csv/Issued_To_Template.csv" style="text-decoration:none;color:#FFF;"><strong style="font-size:16px;">Click here to download CSV template</strong></a>
               <td> <input type="button"  class="submit_btn" onclick=location.href="issue_export.php"  style="background:none;background-image:url(images/export_csv_btn.png); width:87px; height:30px;border:none;margin-left:15px;" /></td>
			...
			<tr>
				...
				<td width="120px;" height="50px"><input type="image" src="images/import_csv_btn.png"  name="location_csv" id="location_csv" value="Import CSV" style="margin-left:15px;" />
