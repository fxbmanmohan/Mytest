			...
			//$upload=move_uploaded_file($_FILES['csvFile']['tmp_name'],$path.$filename);
				...
				for($i=1;$i<$num;$i++) //read second line beacuse first line cover headings
				{
					...
					if(!empty($fieldarray[$i][0]))
					{
						...
						$select="select * from standard_defects where description='".$fieldarray[$i][0]."' AND project_id=".$_SESSION['idp']." and is_deleted=0";
						...
						else
						{
							...
							@$insert="insert into standard_defects (project_id,description,tag,last_modified_date,last_modified_by,created_date,created_by) values (".$_SESSION['idp'].",'".addslashes(trim($desc))."','".addslashes(trim($tags))."',now(),".$builder_id.",now(),".$builder_id.")";
...
<form method="post" name="csvIssueto" id="csvIssueto" enctype="multipart/form-data">
	<table width="690px" border="0" cellspacing="0" cellpadding="3">
		<tr>
			<td colspan="3" align="left"><a href="/csv/Statndard_Defects_Template.csv" style="text-decoration:none;color:#FFF;"><strong style="font-size:16px;">Click here to download CSV template</strong></a></td>
            <td> <input type="button"  class="submit_btn" onclick=location.href="standard_export.php"  style="background:none;background-image:url(images/export_csv_btn.png); width:87px; height:30px;border:none;margin-left:10px;" /></td>
		</tr>   
		<tr>
