...
if(isset($_FILES['csvFile']['tmp_name'])){ // Location/ subloaction import CSV file.
	...
	if(isset($_FILES['csvFile']['name']) && !empty($_FILES['csvFile']['name'])){
		...
		if($ext == 'csv' || $ext == 'CSV'){
			...
			$exData = $obj->selQRYMultiple('task_id, task', 'qa_task_monitoring', 'project_id = '.$_SESSION['idp'].' AND is_deleted = 0');
			$qaTaskData = array();
			foreach($exData as $exDataRow){
				$qaTaskData[] = $exDataRow['task'];
			}
...
//Insert Data Here
				...
				foreach($locData as $lData){
					...
					$query = 'UPDATE qa_task_locations SET location_tree_name = "'.$locNameTree.'", last_modified_date = NOW(), last_modified_by = '.$_SESSION['ww_builder_id'].' WHERE location_id = '.$lData['location_id'];
					mysql_query($query);
				}
		...
		if(data != 'Duplicate task'){
			...
			$('span.demo1').contextMenu('myMenu2', {
				bindings: {
					...
					'edit': function(t) {
						modalPopup(align, top, width, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'location_tree_edit_qa.php?location_id='+t.id, loadingImage);
					},
					'delete': function(t) {
						var r = jConfirm('Do you want to delete location ?, task added on this location also deleted !', null, function(r){
							if (r==true){
...
" /> Table View</label>
		...
		<img class="setOrderImg" src="images/order_wallchart.png" style="margin-bottom:15px;cursor:pointer;" title="Set order for Wallchart Report" />
			<div id="locationsContainer">
				...
				<?php $q = "select location_id, location_title from qa_task_locations where project_id = ".$_SESSION['idp']." and location_parent_id = '0' and is_deleted = '0' order by location_title";
					$re = mysql_query($q);
...
$('.setOrderImg').click(function(){
	...
	modalPopup(align, top, width, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, 'qa_location_set_order.php?projectID='+projectID, loadingImage, temp);
});
...
ul#sortable1 li input { position:absolute; right:15px;}
</style>