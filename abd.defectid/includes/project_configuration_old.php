	...
	if(isset($_FILES['csvFile']['name']) && !empty($_FILES['csvFile']['name']))
	{
		...
		if($ext=='csv' || $ext=='CSV')
		{
			...
			if($upload)
			{
				...
				for($i=1;$i<$num;$i++) //read second line beacuse first line cover headings
				{
					...
					for($j=0;$j< $filedcount;$j++)
					{
						...
						if(!empty($fieldvalue))
						{
							...
							$select="select * from project_locations where location_title='".$fieldvalue."' AND location_parent_id = $parent_id";
							...
							{
								$insert="insert into project_locations (location_parent_id,project_id,location_title) values ($parent_id,".$_SESSION['idp'].",'".$fieldvalue."')";
				...
				$msg1= "<br>Found a total of $lines records in this csv file.";
		}
		else
		{
...
<table width="100%" border="0" align="left" cellpadding="5" cellspacing="0">
		<tr>
				...
				</a><br /><a href="#" <?php if($_GET['sect'] == 'project_configuration123'){echo 'class="left_btn4active"';}?>  class="left_btn4"><br />
