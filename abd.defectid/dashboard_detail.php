...
switch($_GET['type']){
	...
	if(!isset($_SESSION['ww_is_builder']) || $_SESSION['ww_is_builder'] != 1){
		$inspStatusData = $object->getRecordByQuery('SELECT SUM(IF(d.inspection_status= "Open", 1, 0)) AS open,
						...
						FROM 
							project_inspections AS pi inner join (
								...
								issued_to_for_inspections
									WHERE
								project_id = '.$_GET['projID'].' AND
								is_deleted = 0
									GROUP BY inspection_id
							) AS d ON pi.inspection_id = d.inspection_id WHERE pi.project_id = '.$_GET['projID'].' AND pi.inspection_type != "Memo" AND pi.is_deleted = 0');
	}else{
		$inspStatusData = $object->getRecordByQuery('SELECT SUM(IF(d.inspection_status= "Open", 1, 0)) AS open,
							SUM(IF(d.inspection_status="Pending", 1, 0)) AS pending,
							SUM(IF(d.inspection_status="Fixed", 1, 0)) AS fixed,
						...
						FROM 
							project_inspections AS pi inner join (
								...
								is_deleted = 0
									GROUP BY inspection_id
							) AS d ON pi.inspection_id = d.inspection_id WHERE pi.project_id = '.$_GET['projID'].' AND pi.inspection_type != "Memo" AND pi.is_deleted = 0'.$whereConUserRole);
	}
...
<?php if($_GET['type'] == 'location'){?>
	<ul class="labelHolder">
		...
		<?php if(isset($_SESSION['ww_is_builder']) && $_SESSION['ww_is_builder'] != ""){
			$backURL = '?sect=b_full_analysis';
		}else{
			$backURL = '?sect=c_full_analysis';
		}?>
			<a style="float:right;width:87px;" href="<?=$backURL?>">
...
function inspectionsList(statusType, locID){
	...
	<?php }else{?>
			window.location.href = "pms.php?sect=c_defect&frm=dsb&bk=Y&pid="+projectID+"&locid="+locationID+"&sts="+statusType;
	<?php }?>
...
function inspectionsList(statusType, projID){
	...
	<?php }else{?>
		window.location.href = "pms.php?sect=c_defect&frm=dsb&bk=Y&pid="+projectID+"&isst="+issuedTo+"&sts="+statusType;
	<?php }?>
}
<?php }?>
<?php if($_GET['type'] == 'promon'){?>
...
function inspectionsList(statusType){
	...
	$.post('set_session_for_dashboard.php', { projName:projectID, location:locationID, status:statusType, singleID:Math.random() }).done(function(data) {
		...
		if(jsonResult.status){
			...
			<?php }else{?>
					window.location.href = "pms.php?sect=c_defect&bk=Y";
			<?php }?>
		}else{
			jAlert(jsonResult.msg);
		}
