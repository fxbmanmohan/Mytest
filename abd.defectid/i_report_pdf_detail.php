...
$toEmailIDS = array();$mulIssueTo = '';
	...
	if($_REQUEST['issuedTo']!=""){
		...
		for($g=0; $g<$loopMul; $g++){
			if($mulIssueToWhere == ""){
				$mulIssueTo = "'".$isMul[$g]."'";
				$mulIssueToWhere .= " (F.issued_to_name LIKE '".addslashes($isMul[$g])." (%' OR F.issued_to_name LIKE '".$isMul[$g]."') ";
			}else{
				$mulIssueTo .= ", '".$isMul[$g]."'";
...
$html='<table width="98%" border="0" align="center">
		...
		if(!empty($_REQUEST['location']) && empty($_REQUEST['subLocation'])){
			...
			<td width="110" style="font-size:11px;">'.$obj->getDataByKey('project_locations', 'location_id', $_REQUEST['location'], 'location_title').'</td>';$jk++;
		}
		if(!empty($_REQUEST['subLocation']) && !empty($_REQUEST['sub_subLocation'])){
			$html .= '<td width="140" style="font-size:11px;"><b>Location Name : </b></td>
			<td width="110" style="font-size:11px;">'.$obj->getDataByKey('project_locations', 'location_id', $_REQUEST['location'], 'location_title').'</td>';$jk++;
			$html .= '<td width="140" style="font-size:11px;"><b>Sub Location 1 : </b></td>
			<td width="110" style="font-size:11px;">'.$obj->getDataByKey('project_locations', 'location_id', $_REQUEST['subLocation'], 'location_title').'</td>';$jk++; if($jk%2 !=0){$html .= '';}if($jk%2 ==0){$html .= '</tr><tr>';}
			$html .= '<td width="140" style="font-size:11px;"><b>Sub Location 2 : </b></td>
			<td width="110" style="font-size:11px;">'.$obj->getDataByKey('project_locations', 'location_id', $_REQUEST['sub_subLocation'], 'location_title').'</td>';$jk++;
		}else{
			if(!empty($_REQUEST['location']) && !empty($_REQUEST['subLocation'])){
				$html .= '<td width="140" style="font-size:11px;"><b>Location Name : </b></td>
				<td width="110" style="font-size:11px;">'.$obj->getDataByKey('project_locations', 'location_id', $_REQUEST['location'], 'location_title').'</td>';$jk++;
		...
		while( $fi=mysql_fetch_row($ri)){
			...
			if($_POST['issuedTo']!=""){
				...
				for($g=0; $g<$loopMul; $g++){
					if($mulIssueToWhere == ""){
						$mulIssueTo = "'".$isMul[$g]."'";
						$mulIssueToWhere .= " (issued_to_name LIKE '".$_REQUEST['issuedTo']." (%' OR issued_to_name LIKE '".$isMul[$g]."') ";
					}else{
						$mulIssueTo .= ", '".$isMul[$g]."'";
						$mulIssueToWhere .= " OR (issued_to_name LIKE '".$_REQUEST['issuedTo']." (%' OR issued_to_name LIKE '".$isMul[$g]."') ";
					}
				}
					...
					<td width="50%" valign="top" cellspacing="0" cellpadding="0" style="border-bottom:hidden;border-right:hidden;">
						...
						$html .='</td>
							...
							<tr>
								...
								<td width="50%" style="font-size:11px;">'.$issueToData_costAttribute.'</td>
							</tr>
							<tr>
