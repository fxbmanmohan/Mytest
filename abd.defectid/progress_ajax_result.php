...
<form name="allTaskTable" id="allTaskTable">
	...
	while ($row=mysql_fetch_array($rset)) {
		...
		if (mysql_num_rows($rsq) <= 0)
			continue; ?>
			<h3 class="location_header" style="margin:0 0 0 0;float:center;cursor:pointer;text-align:left;width:960px;line-height:30px;" onclick="toggleFolder('f<?php echo $parent_loc_id?>');">
				<span style="text-decoration:none; color:#FFFFFF; font-weight:bold;font-size:large;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;width:800px;display:block;">
	...
	<?php while ($row1 = mysql_fetch_array($rsq)) {
		...
		if ($status == "Complete"){
			...
			$row_color = "#CCCCCC";
				...
				<tr style="color:">
					<td  style="background:<?php echo $row_color; ?>;">
