...
if(isset($_GET['deleteId'])){
	...
	$deleteQry = "UPDATE user_projects SET is_deleted = 1, last_modified_date = NOW(), last_modified_by = ".$_SESSION['ww_builder_id']." WHERE project_id = '".$_SESSION['idp']."' AND user_id = '".$deleteId."'";
	mysql_query($deleteQry);
	$permissionDeleteQry = "update user_permission set is_deleted=1, last_modified_date = NOW(), last_modified_by = ".$_SESSION['ww_builder_id']." WHERE project_id = '".$_SESSION['idp']."' AND user_id = '".$deleteId."'";
...
if(isset($_POST['button1'])){
	if($_POST['no_refresh'] == $_SESSION['no_refresh']){}else{
		...
		if($assignInspector == ''){
			...
			$projectData = $obj->selQRY('project_id, pro_code, project_name, project_type, project_address_line1, project_address_line2, project_suburb, project_state, project_postcode, project_country, resource_type', 'projects', 'project_id = "'.$_SESSION['idp'].'" AND is_deleted = 0');
			...
			if(empty($userProjectData)){
				...
				$updateQry = "UPDATE user_projects SET is_deleted = 0, issued_to = '".$issuedTo."', user_role = '".$assignInspectorRole."', last_modified_date = NOW(), last_modified_by = ".$_SESSION['ww_builder_id']." WHERE project_id = '".$_SESSION['idp']."' AND user_id = '".$assignInspector."'";
				mysql_query($updateQry);
				$permissionUpdateQry = "UPDATE user_permission SET is_deleted = 0, last_modified_date = NOW(), last_modified_by = ".$_SESSION['ww_builder_id']." WHERE project_id = '".$_SESSION['idp']."' AND user_id = '".$assignInspector."'";
...
$hb=base64_encode($_SESSION['hb']);  ?>
		<div id="rightCont" style="float:left;width:700px;">
			<div class="content_hd1" style="width:500px;margin-top:12px;">
				...
				<a href="?sect=add_project_detail&id=<?=$id;?>&hb=<?=$hb;?>" style="display: block;float: none;height: 35px;margin-left: 585px;margin-top: -25px;width: 87px;">
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" >
							<tr>
								<td colspan="3" align="left" style="padding-bottom:15px;">
									<strong style="font-size:16px;color:#ffffff;">&nbsp;&nbsp;Assign User to Project</strong>
								</td>
							</tr>
							<tr>
								<td width="30%" style="color:#FFFFFF;">List of Users</td>
...
<?php if(!empty($newUsers)){
	...
	<?php }
							...
							<tr>
								<td width="30%" style="color:#FFFFFF;">User Role</td>
								<td width="50%" valign="top">
							...
							<tr id="issueToDropDown" style="display:none;">
								<td width="30%" style="color:#FFFFFF;">Issue To</td>
								<td width="50%" valign="top">
								...
								<td width="50%">&nbsp;</td>
								<td width="20%">
									<input name="button1" type="submit" class="submit_btn" id="button1" value="" style="background-image:url(images/submit_btn.png); border:none; width:111px; margin-right:-37px;">
