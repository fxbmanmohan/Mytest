		...
		while( $fi=mysql_fetch_row($ri)){
			...
			if($_POST['issuedTo']!=""){
				...
				for($g=0; $g<$loopMul; $g++){
					if($mulIssueToWhere == ""){
						$mulIssueTo = "'".$isMul[$g]."'";
						$mulIssueToWhere .= " (F.issued_to_name LIKE '".$_REQUEST['issuedTo']." (%' OR F.issued_to_name LIKE '".$isMul[$g]."') ";
					}else{
						$mulIssueTo .= ", '".$isMul[$g]."'";
						$mulIssueToWhere .= " OR  (F.issued_to_name LIKE '".$_REQUEST['issuedTo']." (%' OR F.issued_to_name LIKE '".$isMul[$g]."') ";
					}
				}
				$where.=" AND (".$mulIssueToWhere.") AND F.inspection_id = I.inspection_id";
