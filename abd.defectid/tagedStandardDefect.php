...
if(isset($_REQUEST['uniqueId'])){
	...
	$sdName = $obj->selQRYMultiple('distinct(description), tag', 'standard_defects', 'project_id = "'.$_REQUEST['projectID'].'" AND is_deleted = 0');
	$tagArray = array();
...
#print_r($imagesData);die;
	...
	if(!empty($standardDefects)){
		for($i=0; $i<sizeof($standardDefects); $i++){
			$selOption .= '<li class="clickableLines">'.$standardDefects[$i].'</li>';
		}
	}else{
		$selOption = '<li class="clickableLines">No One Standard Defect Found !</li>';
	}
	echo $selOption;
}?>