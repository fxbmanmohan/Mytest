...
//New Permision Setup
			$qs = "SELECT user_role, issued_to FROM  user_projects WHERE is_deleted = 0  AND user_id = '".$_SESSION['ww_builder_id']."'";
			$rs=$obj->db_query($qs);
...
elseif($sect=='forgot_password'){
	...
	if($email==''){
		...
		if($obj->db_num_rows($obj->db_query($q)) > 0){
			...
			{
				$user_name=$f['comp_userName'];
				$pswd=$f['comp_plainpassword'];
