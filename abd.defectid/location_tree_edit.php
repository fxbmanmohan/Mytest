	...
	if(mysql_num_rows($res) > 1){
		...
		if(mysql_num_rows($res) > 0){
			while($row=mysql_fetch_array($res)){
				$locIdTree = $obj->subLocationsIDS($row['location_id'], ' > ');
				$locNameTree = $obj->subLocations($row['location_id'], ' > ');
				$query = 'UPDATE project_locations SET location_id_tree = "'.$locIdTree.'", location_name_tree = "'.$locNameTree.'", qrcode = "'.$locNameTree.'", last_modified_date = NOW(), last_modified_by = '.$_SESSION['ww_builder']['user_id'].'  WHERE location_id = '.$row['location_id'];
				mysql_query($query);
			}
		}
	}
	...
	if(strpos($_GET['location_id'], 'Id_')){
		...
		<fieldset class="roundCorner">
			...
			<table width="100%" border="0">
				...
				<tr>
					<td align="center" colspan="2" style="padding-top:20px;"><input type="submit" name="submit" id="submit" value="Submit" onclick="editLocation()"  /></td>
				</tr>
			</table>
		</fieldset>
