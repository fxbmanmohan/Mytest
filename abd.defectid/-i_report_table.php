...
<?php }?>
	...
	if(screenID == 'buttoon_qatask'){
<?php if($_SESSION['web_report_quality_assuarance'] == 1){?>
		document.getElementById(screenID).setAttribute("class", "buttoon_qataskActive");
<?php }?>
<?php if($_SESSION['web_report_progress_monitoring'] == 1){?>
		document.getElementById('buttoon_progressMonitoring').setAttribute("class", "buttoon_progressMonitoring");
<?php }?>
...
function downloadPDF(){
	...
	var priority = '';//document.getElementById('priority').value;
		...
		if(reportType == 'pdfDetail'){
			...
			callAjaxDownloadSR ('pdf/i_report_pdf_summary_fast.php', 1, params);
			return;
...
function submitFormQA(){
	try{
		...
		if(subLocationQA1 == ''){ document.getElementById('subLocationQA1Error').style.display = 'block'; return false; }		
		if(subLocationQA2 == ''){ document.getElementById('subLocationQA2Error').style.display = 'block'; return false; }
...
<style> .asdf{ visibility:visible } </style>
	<div id="containerTop" style="width:100%; margin:auto;">
		<div class="content_hd1" style="width:350px;float:left;"></div>
	<?php if($_SESSION['web_report_quality_control'] == 1 && ($_SESSION['web_report_progress_monitoring'] == 1 ||  $_SESSION['web_report_quality_assuarance'] == 1)){?>	
            ...
            <tr>
				...
				<td colspan="2">
					<select name="sortby" id="sortby" class="select_box" style="width:220px;background-image:url(images/selectSpl.png);">
						...
						<option value="issued_to_name">Issued To</option>
					</select>
				</td>
...
<?php if($_SESSION['web_report_quality_assuarance'] == 1){?>
		...
		<form action="pdf/i_report_pdf_summary.php" name="form_progress_monitoring" id="form_progress_monitoring" target="_blank" method="post">
			<tr>
				...
				<td align="left" valign="top" nowrap="nowrap" style="color:#FFFFFF;">Report Type <span class="reqire">*</span></td>
				<td colspan="2" valign="top">
					<select name="reportTypeQA" id="reportTypeQA"  class="select_box" style="width:220px;background-image:url(images/selectSpl.png);" onChange="resetIds();">
						<option value="">Select</option>
<?php if($_SESSION['web_report_qa_task'] == 1){?>
						<option value="qaReport">QA Report</option>
<?php }?>
<?php if($_SESSION['web_report_non_conformance'] == 1){?>
						<option value="nonConformance">Non Conformance Report</option>
<?php }?>
					</select>
			...
			<tr id="row1">
				...
				<td align="left" valign="top" nowrap="nowrap" style="color:#FFFFFF;">Sub Location 1 <span class="reqire">*</span></td>
				<td colspan="2">
			...
			<tr id="row1">
				<td align="left" valign="top" nowrap="nowrap" style="color:#FFFFFF;">Sub Location 2 <span class="reqire">*</span></td>
				<td colspan="2">
...
<div id="userRole"></div>
<script type="text/javascript"> function closePopUp(){ closePopup(300); } </script>
</body>
</html>