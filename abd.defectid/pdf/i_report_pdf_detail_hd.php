...
*/
	...
	if ($_REQUEST["sortby"]){
		if ($_REQUEST["sortby"] == "issued_to_name")
			$orderby = "order by F.issued_to_name, I.location_id";
...
if(isset($pdf)){
	...
	$pdf->image("../company_logo/logo.png", 250, 835, 100);
	$pdf->close_object(); 
		...
		while( $fi=mysql_fetch_row($ri)){
			...
			if($_POST['issuedTo']!=""){
				...
				for($g=0; $g<$loopMul; $g++){
					if($mulIssueToWhere == ""){
						$mulIssueTo = "'".$isMul[$g]."'";
						$mulIssueToWhere .= " (issued_to_name LIKE '".$isMul[$g]." (%' OR issued_to_name LIKE '".$isMul[$g]."') ";
					}else{
						$mulIssueTo .= ", '".$isMul[$g]."'";
						$mulIssueToWhere .= " OR  (issued_to_name LIKE '".$isMul[$g]." (%' OR issued_to_name LIKE '".$isMul[$g]."') ";
					}
				}
				$where.=" AND (".$mulIssueToWhere.") ";
...
$obj->resizeImages('../inspections/photo/'.$images[0]['graphic_name'], 320, 320, '../inspections/photo/photo_detail/'.$images[0]['graphic_name']);
				if(file_exists('../inspections/photo/'.$images_path.$images[0]['graphic_name'])){
					...
					$html .='<img src="../inspections/photo/'.$images[0]['graphic_name'].'" style="width:320px;" />';
				}
			}
			$html .='</td><td colspan="3" align="center" style="width:330px;height:'. $iHeight.'">';
			if(isset($images[1]) && $images[1]['graphic_name'] != ''){
$obj->resizeImages('../inspections/photo/'.$images[1]['graphic_name'], 320, 320, '../inspections/photo/photo_detail/'.$images[1]['graphic_name']);
				if(file_exists('../inspections/photo/'.$images_path.$images[1]['graphic_name'])){
					...
					$html .='<img src="../inspections/photo/'.$images[1]['graphic_name'].'"  style="width:320px;"/>';
				}
			}
...
$obj->resizeImages('../inspections/drawing/'.$drawing[0]['graphic_name'], 320, 320, '../inspections/drawing/drawing_detail/'.$drawing[0]['graphic_name']);
				if(file_exists('../inspections/drawing/'.$dimages_path.$drawing[0]['graphic_name'])){
					...
					$html .='<img src="../inspections/drawing/'.$drawing[0]['graphic_name'].'"  style="width:320px;" />';
				}
			}
			$html .='</td>
				</tr>
