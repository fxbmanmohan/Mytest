...
#		}
		$locNameIDs = $_REQUEST['location'].','.$_REQUEST['subLocation'].','.$_REQUEST['sub_subLocation'];
	}else{
		if(!empty($_REQUEST['location']) && !empty($_REQUEST['subLocation'])){
			$postCount++;
/*			if($subwhere != ''){
...
#			}
			$locNameIDs = $_REQUEST['location'].','.$_REQUEST['subLocation'];
		}
	}
...
//Top Header Sectiond Start Here
	...
	$pdf->Image('../company_logo/logo.png', 200, 5, 65, 0, 'png');
	$pdf->Ln(8);
...
//Data Section Start Here
	...
	while($fi=mysql_fetch_assoc($ri)){
		...
		$pdf->Row(array($fi['InspectionId'], $locations, $fi['Description'], $fi['Note'], $fi['InspectedBy'], stripslashes(date("d/m/y", strtotime($fi["DateRaised"]))), stripslashes( $fi['RaisedBy']), $issueToData_issueToName, $issueToData_fixedByDate, $issueToData_status, $image0, $image1, $drawing_image, $signoff_image));
	}
