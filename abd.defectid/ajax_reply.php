...
#echo $q; die;
					...
					$projectData = $object->selQRY('project_id, pro_code, project_name, project_type, project_address_line1, project_address_line2, project_suburb, project_state, project_postcode, project_country, resource_type', 'projects', 'project_id = "'.$_POST['proId'].'" and is_deleted = 0');
...
elseif($sect=='drawing_mgmt_edit'){
	if(isset($_POST['drawingID'])){
		...
		if(trim($_POST['drawingImage']) != '' && trim($_POST['drawingImageThumb']) != ''){
			$drawImageName = trim(addslashes($_POST['drawingImage']));
			$drawingImageThumb = trim(addslashes($_POST['drawingImageThumb']));
			
			 $inssertQRY = "UPDATE draw_mgmt_images SET
							project_id = '".$_SESSION['idp']."',
							draw_mgmt_images_title = '".$drawTitle."',
							draw_mgmt_images_name = '".$drawImageName."',
							draw_mgmt_images_thumbnail = '".$drawingImageThumb."',
		...
		if($err_msg == ''){
			...
			$msg = 'Drawing image uploaded and data updated successfully !';
			echo json_encode(array("msg"=>$msg,"status"=>true));die;
		}else{
			echo json_encode(array("msg"=>$msg,"status"=>true));die;
		}
	}else{
