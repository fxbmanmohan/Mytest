<?php //Header Secttion for include and objects 
				...
				while($file = readdir($folder)){
					if(in_array(substr(strtolower($file), strrpos($file,".") + 1), $file_types)){
						if($file == 'inspection_graphics.json'){
							...
							$updateCount = 0;
							$inspGraphicArr = array();//Store ImageData As per Inspections.
...
//Condition to stop duplicate records Dated : 22-03-2013	
									...
									if(empty($checkData)){
										...
										if(is_array($inspGraphicArr[$jDataGraphic['inspection_id']])){
											$inspGraphicArr[$jDataGraphic['inspection_id']][] = array($jDataGraphic['graphic_name'], $jDataGraphic['graphic_type']);
										}else{
											$inspGraphicArr[$jDataGraphic['inspection_id']] = array();
											$inspGraphicArr[$jDataGraphic['inspection_id']][] = array($jDataGraphic['graphic_name'], $jDataGraphic['graphic_type']);
										}
									}else{
										$arr['id'] = $jDataGraphic['graphic_id'];
...
//Condition to stop duplicate records Dated : 22-03-2013	
						...
						if($file == 'issued_to_for_inspections.json'){
							...
							$updateCount = 0;
							$inspIssueArr = array();//Store IssuestoData As per Inspections.
...
//Condition to stop duplicate records Dated : 22-03-2013	
									...
									if(empty($checkData)){
										...
										if(is_array($inspIssueArr[$jDataInspection['inspection_id']])){
											$inspIssueArr[$jDataInspection['inspection_id']][] = array($jDataInspection['issued_to_name'], $jDataInspection['project_id'], $jDataInspection['inspection_status']);
										}else{
											$inspIssueArr[$jDataInspection['inspection_id']] = array();
											$inspIssueArr[$jDataInspection['inspection_id']][] = array($jDataInspection['issued_to_name'], $jDataInspection['project_id'], $jDataInspection['inspection_status']);
										}
										//$issuedToDataArr[$jDataInspection['inspection_id']][] = $jDataInspection;
									}else{
										$arr['id'] = $jDataInspection['id'];
...
//Condition to stop duplicate records Dated : 22-03-2013	
						...
						if($file == 'project_inspections.json'){
							...
							$updateCount = 0;
							$inspDataArr = array();//Store InspData As per Inspections.
...
//Condition to stop duplicate records Dated : 22-03-2013	
									...
									if(empty($checkData)){
										...
										$inspDataArr[$jDataPinspections['inspection_id']] = $jDataPinspections;
										$projectArr[] = $jDataPinspections['project_id'];
										//$inspectionDataArr[$jDataPinspections['inspection_id']] = $jDataPinspections;
									}else{
										$arr['id'] = $jDataPinspections['inspection_id'];
										$arr['global_id'] = $jDataPinspections['inspection_id'];
										$data[$tableNamePinspections][] = $arr; 
...
function insertEmailToDB(){
	global $db; 
	global $userId; 
	global $projectArr;
	global $inspectionDataArr;
	global $inspGraphicArr;
	global $inspIssueArr;
	global $inspDataArr;
	
	//get issued to ID
	$issedInspEmailArr = array();
	$nameEmailArr = array();

	foreach($inspIssueArr as $inspID => $issuedToArr){
		$issedToEmailArr = array();
		foreach($issuedToArr as $isToArr){
			if(strpos($isToArr[0], ' (') !== false){
				$teArr = explode(' (', $isToArr[0]);
				$cName = $teArr[0];

				$isName = trim($teArr[1], ')');
				$isData = $db->selQRYMultiple("issue_to_email", "inspection_issue_to", "is_deleted = 0 AND project_id = ".$isToArr[1]." AND company_name = '".$isName."' AND issue_to_name = '".$cName."'");
				$issedToEmailArr[$isToArr[0]] = $isData[0]['issue_to_email'];
				$nameEmailArr[$isToArr[0]] = array($isData[0]['issue_to_email'], $cName); 
			}else{
				$isData = $db->selQRYMultiple("issue_to_email", "inspection_issue_to", "is_deleted = 0 AND project_id = ".$isToArr[1]." AND issue_to_name = '".$isToArr[0]."'");

				$issedToEmailArr[$isToArr[0]] = $isData[0]['issue_to_email'];
				$nameEmailArr[$isToArr[0]] = array($isData[0]['issue_to_email'], $isToArr[0]); 
			}
		}
		$issedInspEmailArr[$inspID] = $issedToEmailArr;
	}
	
	$issueNameInspArr = array();
	
	foreach($issedInspEmailArr as $isnpID=>$isNameArr){
		foreach($isNameArr as $isName=>$isEmail){
			if(is_array($issueNameInspArr[$isName])){
				$issueNameInspArr[$isName][] = $isnpID;
			}else{
				$issueNameInspArr[$isName] = array();
				$issueNameInspArr[$isName][] = $isnpID;
			}
		}
	}
	//Get the Project List
	$projects = $db->selQRYMultiple("project_id, project_name", "projects", "is_deleted = 0 AND project_id IN (".join(",", $projectArr).")");
	$projNameArr  = array();
	foreach($projects as $projVal){
		$projNameArr[$projVal['project_id']] = $projVal['project_name']; 
	}
	//User Email Address who is syncing
	$usrData = $db->selQRYMultiple("user_id, user_email, user_fullname", "user", "is_deleted = 0 AND user_id = ".$userId);

	$usrNameArr = array();
	foreach($usrData as $usrDt){
		$usrNameArr[$usrDt['user_id']] = array($usrDt['user_email'], $usrDt['user_fullname']); 
	}
	
	//Start Sending Emails
	if(isset($issueNameInspArr) && !empty($issueNameInspArr)){// Prepare html emal content
		$path = 'http://'.str_replace('/', '', str_replace('http://', '', DOMAIN));
		
		foreach($issueNameInspArr as $isName=>$inspArr){
			$attArr = array();
			$emailContent = "Hello,<br/> <br />
				You have new Inspection(s) on Project: ".$projNameArr[$inspDataArr[$inspArr[0]]['project_id']].".<br /> <br />";
				foreach($inspArr as $inspID){

					//Create Images Array Start Here
					foreach($inspGraphicArr[$inspID] as $inGrphArr){
						if($inGrphArr[1] == "images"){
							$imgNameArr = explode('_', $inGrphArr[0]);
							$attArr[] = array(
									'source_file' => '../inspections/photo/'.$inGrphArr[0],
									'destination' => $imgNameArr[0].'_'.str_replace(' ', '_', $inspDataArr[$inspID]['inspection_description']).'_'.end($imgNameArr).'.png'
								);
						}else{
							$imgNameArr = explode('_', $inGrphArr[0]);
							$attArr[] = array(
									'source_file' => '../inspections/drawing/'.$inGrphArr[0],
									'destination' => $imgNameArr[0].'_'.str_replace(' ', '_', $inspDataArr[$inspID]['inspection_description']).'_'.end($imgNameArr).'.png'
								);
						}
					}
					
					$emailContent .= "Location: ".$inspDataArr[$inspID]['inspection_location']."<br />
					Raised Date: ".date('d/m/Y', strtotime($inspDataArr[$inspID]['inspection_date_raised']))."<br />
					Description: ".$inspDataArr[$inspID]['inspection_description']."<br />
					Status: ".$inspIssueArr[$inspID][0][2]."<br /><br />
					Inspected By: ".$inspDataArr[$inspID]['inspection_inspected_by']."<br /><br />";
				}
				$emailContent .= "You can access the DefectID system to view or manage this and all of your issues. Please contact ABD Group to find out how to do this. Click on the below link to access the login screen to see your open defects if you have a login.<br /><br />
					<a href='".$path."/pms.php?sect=i_defect&frm=dsb&bk=Y&pid=".$inspDataArr[$inspID]['project_id']."&sts=Open'>".$path."/pms.php?sect=i_defect&frm=dsb&bk=Y&pid=".$inspDataArr[$inspID]['project_id']."&sts=Open</a><br /><br />
					Yours sincerely,<br /><br />
					ABD Group";
			#$emailContent = mysql_real_escape_string(nl2br(htmlentities($emailContent, ENT_QUOTES, 'UTF-8')));
			$emailContent = htmlspecialchars($emailContent);
			$emInspQry = "INSERT INTO email_status_table SET
								user_id = ".$userId.",
								module_type = 'inspections',
								reciever_name = '".$nameEmailArr[$isName][1]."',
								email_to = '".$nameEmailArr[$isName][0]."',
								cc = '".$usrNameArr[$userId][0]."',
								content = '".addslashes($emailContent)."',
								email_status = 0,
								subject = 'New Inspection Added',
								attachment = '".addslashes(serialize($attArr))."',
								project_id = '".$inspDataArr[$inspArr[0]]['project_id']."',
								last_modified_date = NOW(),
								last_modified_by = '".$userId."',
								created_date = NOW(),
								created_by = '".$userId."'";
			mysql_query($emInspQry);
			//call back function call here
		}
	}
}?>