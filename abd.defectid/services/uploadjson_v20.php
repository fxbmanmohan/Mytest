...
$projectIds = array();
$inspectionDataArr = array();
$issuedToDataArr = array();
...
//Condition to stop duplicate records Dated : 22-03-2013	
									...
									if(empty($checkData)){
										...
										$projectIds[$jDataPinspections['project_id']] = $jDataPinspections['project_id'];
...
// Send Email To Sub Contractors
function sendEmailToSubContractors(){
	require_once("class.phpmailer.php");
	global $db; 
	global $projectIds;
	global $inspectionDataArr;
	global $issuedToDataArr;
	//get issued to ID
	$issedInspEmailArr = array();
	$nameEmailArr = array();
	foreach($issuedToDataArr as $inspID => $issuedToArr){
		$issedToEmailArr = array();
		foreach($issuedToArr as $isToArr){
			if(strpos($isToArr, ' (') !== false){
				$teArr = explode(' (', $isToArr['issued_to_name']);
				$cName = $teArr[0];
				$isName = trim($teArr[1], ')');
				$isData = $db->selQRYMultiple("issue_to_email", "inspection_issue_to", "is_deleted = 0 AND project_id = ".$isToArr['project_id']." AND company_name = '".$isName."' AND issue_to_name = '".$cName."'");
				$issedToEmailArr[$isToArr['issued_to_name']] = $isData[0]['issue_to_email']; 
				$nameEmailArr[$isToArr['issued_to_name']] = array($isData[0]['issue_to_email'], $cName); 
			}else{
				$isData = $db->selQRYMultiple("issue_to_email", "inspection_issue_to", "is_deleted = 0 AND project_id = ".$isToArr['project_id']." AND issue_to_name = '".$isToArr['issued_to_name']."'");
				$issedToEmailArr[$isToArr['issued_to_name']] = $isData[0]['issue_to_email']; 
				$nameEmailArr[$isToArr['issued_to_name']] = array($isData[0]['issue_to_email'], $isToArr['issued_to_name']); 
			}
		}
	...
	$projects = $db->selQRYMultiple("project_id, project_name", "projects", "is_deleted = 0 AND project_id IN (".join(",", $projectIds).")");
	...
	if(isset($issueNameInspArr) && !empty($issueNameInspArr)){// Prepare html emal content
		...
		foreach($issueNameInspArr as $isName=>$inspArr){
			$emailContent = "Hello,<br/> <br />
				You have new Inspection(s) on Project: ".$projNameArr[$inspectionDataArr[$inspID]['project_id']].".<br /> <br />";
				foreach($inspArr as $inspID){
					$emailContent .= "Location: ".$inspectionDataArr[$inspID]['inspection_location']."<br />
					Raised Date: ".date('d/m/Y', strtotime($inspectionDataArr[$inspID]['inspection_date_raised']))."<br />
					Description: ".$inspectionDataArr[$inspID]['inspection_description']."<br />
					Status: ".$issuedToDataArr[$inspID][0]['inspection_status']."<br />
					Inspected By: ".$inspectionDataArr[$inspID]['inspection_inspected_by']."<br /><br />";
				}
				$emailContent .= "You can access the DefectID system to view or manage this and all of your issues. Please contact ABD Group to find out how to do this. Click on the below link to access the login screen to see your open defects if you have a login.<br /><br />
					<a href='".$path."/pms.php?sect=i_defect&frm=dsb&bk=Y&pid=".$inspectionDataArr[$inspID]['project_id']."&sts=Open'>".$path."/pms.php?sect=i_defect&frm=dsb&bk=Y&pid=".$inspectionDataArr[$inspID]['project_id']."&sts=Open</a><br /><br />
					Yours sincerely,<br /><br />
					ABD Group";
			$htmlData = $emailContent;
			//Send Emial Here
			$mail = new PHPMailer();
			$mail->Host       = "pod51022.outlook.com"; //"smtp.gmail.com";      // sets GMAIL as the SMTP server
			...
			$mail->AddReplyTo("noreply@abd.defectid.com", "No Replay");
			$mail->SetFrom("noreply@abd.defectid.com", "ABD DefectID");
			
			$path = 'http://'.str_replace('/', '', str_replace('http://', '', DOMAIN));
			
			$mail->Subject = "New Inspection Added";
			$mail->IsHTML(true);

			$mail->AddAddress($nameEmailArr[$isName][0], $nameEmailArr[$isName][1]); // To
			$mail->AddBCC("sharma.gaurav@fxbytes.com", "Gaurav Sharma");//send to mail heree
			...
			if(!$mail->Send()) {
				...
				#echo $nameEmailArr[$isName][0]." Email Sent!".$nameEmailArr[$isName][1];
			}
			$mail->ClearAddresses();
