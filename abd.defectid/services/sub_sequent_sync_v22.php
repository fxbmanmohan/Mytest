...
$inspArray = array();
	foreach($projectId as $project){
		...
		if ($new_project[$project['project_id']] == 1){
			$rsProjectInspections = mysql_query("SELECT ".$selProjectInspections." FROM project_inspections  WHERE project_id = '".$project['project_id']."' and is_deleted=0 ".$acceptConNewProj);
		}else{
			$rsProjectInspections = mysql_query("SELECT ".$selProjectInspections." FROM project_inspections  WHERE project_id = '".$project['project_id']."' and is_deleted=0 ".$acceptCon." ".$refuseCon);
		}
...
//inspection graphic table data here
		...
		if ($new_project[$project['project_id']] == 1){
			$rsInspectionInspectedBy = mysql_query("SELECT ".$selInspectionInspectedBy." FROM inspection_check_list WHERE project_id = '".$project['project_id']."' is_deleted=0  AND inspection_id IN (".$inspArray[$project['project_id']].")");
		}else{
