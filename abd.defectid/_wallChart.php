...
if(isset($_REQUEST['uniqueId'])){
	if(!empty($_REQUEST['projName'])){
		...
		if(!empty($querySubLoc)){
			$i = 1;
                        $pagecount = 0;
                        $html_location[$pagecount] = '<tr><td style="width:190px">Status</td><td style="width:60px">'.date('d/M/Y').'</td>';
                        $html_sub_location[$pagecount] = '<tr><td style="width:190px"><strong>TASK</strong></td><td style="width:60px">';
			foreach($querySubLoc as $subLoc){
				...
				$html_location[ $pagecount] .= '<td style="width:100px">'.$parent_location[$subLoc['location_parent_id']].'</td>';
				$html_sub_location[ $pagecount] .= '<td style="width:100px">'.$subLoc['location_title'].'</td>';
...
//Issue to for progress report
			...
			foreach($queryTaskData as $taskData){
                            if ($issue_flag)
                            {
                                $html_tds[$vpagecount] [$hpagecount] .= '<td><div>'.$tasks['task'].'</div>';
                                ...
                                foreach($queryIssuedToData as $issue_to)
                                {
                                    if ($issue_to_name=="")
                                        ...
                                        $issue_to_name .= "<br/>".$issue_to["issued_to_name"];
                                }
                                $html_tds[$vpagecount] [$hpagecount] .= '<div>'.$issue_to_name.'</div>';
                                $html_tds[$vpagecount] [$hpagecount] .= "</td><td><div style='border-bottom:1px solid black'>Start</div><div style='border-bottom:1px solid black'>Finish</div><div>Status</div></td>";
                                $issue_flag = false;
                            }
                                ...
                                if($postion != $i){
                                        for ($j=$i; $j<$postion;$j++){
                                                $i++;
                                                $html_tds [$vpagecount] [$hpagecount] .= "<td>&nbsp;</td>";
                                        }
                                }
                                if (intval($i/27)==1)
                                {
                                ...
                                $html_tds [$vpagecount] [$hpagecount] .= "<td>
                                                                <div style='border-bottom:1px solid black'>".date('d-M-Y', strtotime($taskData['start_date']))."</div>
                                                                <div style='border-bottom:1px solid black'>".date('d-M-Y', strtotime($taskData['end_date']))."</div>
...
foreach($html_tds as $html_h){
    for ($i=0;$i<count($html_h);$i++){
		...
		if($page==1){	$html .= $headerHtml;	}else{	$html .= '<div style="height:120px;"></div>'; }
		$html .= '<table '.$tableWidth.' border="1" class="collapse" cellspacing="0" cellpadding="0">';
        if ($flag){
...
if($obj->create_zip($zipFileArray, 'report_pdf/'.$owner_id.'/'.$zipName)){
	echo '<br clear="all" /><div style="margin-left:10px;">Wall Chart Report Generated<a href="report_pdf/'.$owner_id.'/'.$zipName.'" target="_blank" class="view_btn"></a></div>';
}else{
